...
Our workflow was done prior to publication of this so, original work up of individual/team researcher qualities done in Stata
...
The above files contain personal identifiers and are therefore not permitted for public use. 
...
It is necessary to convert them into long form for plotting and analysis. Also, note that u_teamid2 keeps the two teams that submitted separate analyses by wave as distinct teams for now (team 59 and 81)
...
Remove teams that did not complete the replication
...
```{r dataout}
dataout <- dplyr::select(cri_long, Exp1, u_teamid, verif, deviance_abs)

dataout <- dataout %>%
  left_join(dplyr::select(ungroup(cri_cur_long), u_teamid, verif, deviance_abs, routine), by = "u_teamid") %>%
  mutate(Exp1 = as.numeric(Exp1)) %>%
  ungroup()
...
datao <- datao %>%
  mutate(verif.x = round(verif.x, 3),
         verif.y = round(verif.y, 3),
         deviance_abs.x = round(deviance_abs.x, 3),
         deviance_abs.y = round(deviance_abs.y, 3)) %>%
  arrange(Exp1, u_teamid)

write.csv(datao, here::here("results","dataout.csv"), row.names = F)
```
...
This file is part of [https://github.com/nbreznau/how_many_replicators](https://github.com/nbreznau/how_many_replicators), the reproduction materials for [*How Many Replicators Does It Take to Achieve Reliability? Investigating Researcher Variability in a Crowdsourced Replication*](https://osf.io/preprints/socarxiv/j7qta/).
