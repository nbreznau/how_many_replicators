...
cri_long <- cri_long %>%
  mutate(deviance_ratio = case_when(
         or > orig_or ~ round((or/orig_or)-1,3),
         or < orig_or ~ round((orig_or/or*-1)+1,3),
         .default = 0.000)
  )
...
Some of the results lead to outlandish ratio scores of 1000 times difference or more, mostly because of two teams reporting coefficients instead of odds ratios. This gets fixed in the trimmed and curated versions, but not here. This creates a huge outlier bias in the regressions. Therefore, we trim at ratios of 7 in either direction, because this is basically where teams 77 and 80 start (the ones who made reporting errors). 
...
The file variable_labels.R contains labels, so the user can use haven labels (and for example export to Stata if desired)
...
rm(cri_cur, qual_out, imp, a, b, m1, team_trim)
