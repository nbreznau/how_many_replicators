...
```{r fig2newc}
f2colors32 <- c("#482677FF", "#29AF7FFF")
...
```{r fig2newbuild}
agg_png(here::here("results","Fig2_New.png"), res = 144, height = 1000, width = 1800)

ggarrange(fig2aN, fig2bN, fig2aN_dev, fig2bN_dev, nrow = 2, ncol = 2, heights = c(2,1))

dev.off()

knitr::include_graphics(here::here("results","Fig2_New.png"))
```
...
desc <- as.data.frame(matrix(nrow = 36, ncol = 8))

desc[1,] <- c("","","Team-Level","","","Effect-Level","","")

desc[2,] <- c("Variables","Measurement","Transparent","Opaque", "Pooled", "Transparent","Opaque", "Pooled")

desc[,1] <- c("", "Variables","Cases", "Raw Replication Results", "Verification", "Exact Verification", "Deviance", "Deviance, SD", "Trimmed Replication Results", "Verification", "Exact Verification", "Deviance", "Deviance, SD","Curated Replication Results", "Verification", "Exact Verification", "Deviance", "Deviance, SD", "Independent Variables", "Stata", "SD", "Sociology Degree","SD", "Stats-Skill","SD", "Difficulty","SD", "Team Size", "SD","Qualitative Categories", "Mistake", "Procedural", "Mistake-Procedural", "Missing Component", "Interpretational", "Questionable Methods Competencies")
...
desc[3:36,6] <- c(sum(!is.na(cri_long$verif[cri_long$Exp1 == 1])),"", 
...
desc[3:36,7] <- c(sum(!is.na(cri_long$verif[cri_long$Exp1 == 0])),"", 
...
desc[3:36,8] <- c(sum(!is.na(cri_long$verif)),"", 
...
desc[10:13,5] <- as.numeric(c(t(cri_long_trim_pool[1,1:3]),cri_long_trim_pool[1,9]))
desc[10:13,3] <- as.numeric(c(t(cri_long_trim_pool_t[1,1:3]),cri_long_trim_pool_t[1,9]))
desc[10:13,4] <- as.numeric(c(t(cri_long_trim_pool_o[1,1:3]),cri_long_trim_pool_o[1,9]))

desc[15:18,5] <- as.numeric(c(t(cri_long_cur_pool[1,1:3]),cri_long_cur_pool[1,9]))
desc[15:18,3] <- as.numeric(c(t(cri_long_cur_pool_t[1,1:3]),cri_long_cur_pool_t[1,9]))
desc[15:18,4] <- as.numeric(c(t(cri_long_cur_pool_o[1,1:3]),cri_long_cur_pool_o[1,9]))
