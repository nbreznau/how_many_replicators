...
|*Exact Replication* (exact)|The value of the replication odd-ratio is identical to original within 1% (0.01 as a ratio) |1=exact, 0=not|
|*Replication Error* (deviance_abs)|The absolute deviation of the replicated odd-ratio from the original odds-ratio expressed as a ratio | continuous measure starting from exact (=0) and increasing in positive values to measure the error |
...
pacman::p_load("dplyr", "readr", "lattice", "tidyr", "readxl", "knitr", "boot", "ragg", "kableExtra", "ggpubr","lme4", "jtools","sjPlot", "sjmisc", "sjlabelled", "rvest", "lavaan", "lavaanPlot", "see", "ggtext", "specr", "ggtext","extrafont", "ggridges")
...
# trim deviance_abs at 0.25, for density plotting purposes
cri_long <- cri_long %>%
  mutate(deviance_abs1 = case_when(
    deviance_abs >= 0.15 ~ 0.15,
    .default = deviance_abs
  ))
...
fig2aN <- ggplot(data = fig2t[c(1,2,5,6),]) +
  ...
  annotate("text", x = c(1.2,4.2), y = c(-12, -12), 
           ...
           family = "Times New Roman",
           color = "gray30") +
  annotate("text", x = c(2,3), y = c(1, 1), 
           label = c("Transparent Group (TG)", "Opaque Group (OG)"),
           hjust = 0, 
           family = "Times New Roman",
           size = 5) +
  theme_classic() +
  theme(text = element_text(family = "Times New Roman", size = 14),
        ...
        axis.title.x = element_text(size = 16, hjust = 0, vjust = -0.5),
...
fig2bN <- ggplot(data = fig2bt[c(1,2,5,6),]) +
  ...
  scale_x_reverse() +
  annotate("text", x = c(1.2,4.2), y = c(-12, -12), 
           label = c("VERIFICATION* (95%+)", "EXACT REPLICATION* (95%+)"), 
           hjust = 0.05, 
           vjust = -0.05,
           size = 6, 
           fontface = "bold", 
...
agg_png(here::here("results", "Fig1c.png"), res = 72, height = 300, width = 400)



fig2aN_dev <- ggplot() +
  coord_flip() +
  geom_point(aes(y = deviance_abs_fn1, x = c(2,4), color = group, size = 5), 
             data = fig2t[c(1,2),]) +
  scale_x_reverse() +
  geom_errorbar(aes(ymin = deviance_abs_fn1-(2.58*(deviance_abs_fn2/sqrt(n_fn1))), ymax = deviance_abs_fn1+(2.58*(deviance_abs_fn2/sqrt(n_fn1))), x = c(2,4),), 
                color = "grey30", 
                width = 0.3, 
                data = fig2t[c(1,2),]) +
  scale_color_manual(values = f2colors32,
                    guide = guide_legend(reverse = TRUE)) +
  geom_text(aes(y = fig3t$result[c(1,2)], x = c(1.7,3.7),     
                label = round(fig3t$result[c(1,2)],2)), 
            family = "Times New Roman",
            #hjust = 0,
            size = 5) + 
  theme_classic() +
  ylab("Absolute Difference of Replication Odds Ratios,\nAverage by Experimental Group") +
  xlab(" ") +
  #labs(title = "A. Raw Effect-Level Results, N = 3,742") +
  coord_flip(xlim = c(5.5,1), 
             ylim = c(-0.12,1.00),  
             clip = "on") +
  scale_x_reverse() +
  annotate("text", x = 1.2, 
           ...
           family = "Times New Roman",
           color = "gray30") +
  annotate("text", x = c(2,4), y = c(-0.1,-0.1), 
           label = c("TG", "OG"),
           family = "Times New Roman",
           hjust = 0.1, 
           size = 5) +
...
agg_png(here::here("results", "Fig1d.png"), res = 72, height = 300, width = 400)
fig2bN_dev <- ggplot(data = fig2b[c(1,2),]) +
  coord_flip() +
  geom_point(aes(y = deviance_fn1, x = c(2,4), color = group, size = 5)) +
  geom_errorbar(aes(ymin = deviance_fn1-(1.96*(deviance_fn2/sqrt(n_fn1))), ymax = deviance_fn1+(1.96*(deviance_fn2/sqrt(n_fn1))), x = c(2,4),), color = "grey30", width = 0.3) +
  scale_x_reverse() +
  scale_color_manual(values = f2colors32,
                    guide = guide_legend(reverse = TRUE)) +
  geom_text(aes(y = round(fig3bt$result[c(1,2)],2), x = c(1.7,3.7),                
                label = round(fig3bt$result[c(1,2)],2)), 
            family = "Times New Roman",
            #hjust = 0,
            size = 5) + 
  theme_classic() +
  ylab("Absolute Difference in Replication Odds Ratios,\nTeam-Level Average by Experimental Group") +
  xlab(" ") +
  coord_flip(xlim = c(5.5,1), 
             ylim = c(-0.12,1),  
             clip = "on") +
  ...
  annotate("text", x = 1.2, 
           ...
           family = "Times New Roman",
           color = "gray30") +
  #geom_segment(aes(x = 5.5, y = 0, xend = 1.5, yend = 0), color = "grey30", linetype = "dashed") +
  theme_classic() +
  theme(text = element_text(family = "Times New Roman"),
        axis.line.y = element_blank(),
        axis.ticks.y = element_blank(),
...
agg_png(here::here("results", "Fig1csupa.png"), res = 72, height = 75, width = 400)
fig1csupa <- ggplot() +
  geom_density(aes(x = deviance_abs1),
               color = f2colors32[1],
               fill = f2colors32[1],
               data = subset(cri_long, u_expgroup1 == "0")) +
  annotate("text", x = 0.125, y = 18,
           label = c("Higher values recoded\nfor plotting"),
           size = 4,
           family = "Times New Roman",
           color = "gray22",
           lineheight = 0.9) +
...
knitr::include_graphics(here::here("results", "Fig1csupa.png"))
...
agg_png(here::here("results", "Fig1csupb.png"), res = 72, height = 100, width = 400)
fig1csupb <- ggplot() +
  geom_density(aes(x = deviance_abs1),
               color = f2colors32[2],
               fill = f2colors32[2],
               data = subset(cri_long, u_expgroup1 == "1")) +
  # annotate("text", x = 0.125, y = 18,
  #          label = c("Higher values recoded\nfor plotting"),
  #          size = 4,
  #          family = "Times New Roman",
...
knitr::include_graphics(here::here("results", "Fig1csupb.png"))
...
agg_png(here::here("results", "Fig1dsupa.png"), res = 72, height = 75, width = 400)
fig1dsupa <- ggplot() +
  geom_density(aes(x = dev_m),
               color = f2colors32[1],
               fill = f2colors32[1],
               data = subset(cri_team_orig_c, exp == 1)) +
  # annotate("text", x = 0.125, y = 18,
  #          label = c("Higher values recoded\nfor plotting"),
  #          size = 4,
  #          family = "Times New Roman",
...
knitr::include_graphics(here::here("results", "Fig1dsupa.png"))
...
agg_png(here::here("results", "Fig1dsupb.png"), res = 72, height = 100, width = 400)
fig1dsupa <- ggplot() +
  geom_density(aes(x = dev_m),
               color = f2colors32[2],
               fill = f2colors32[2],
               data = subset(cri_team_orig_c, exp == 0)) +
  # annotate("text", x = 0.125, y = 18,
  #          label = c("Higher values recoded\nfor plotting"),
  #          size = 4,
  #          family = "Times New Roman",
...
knitr::include_graphics(here::here("results", "Fig1dsupb.png"))
...
In a previous version of Figure 1 we included the percent of variance that occurred between groups, but this became superfluous to any arguments made in the paper, so we removed it. We leave the code here in case readers are interested. 
