...
fig2aN_dev <- ggplot(data = fig2t[c(1,2),]) +
  ...
  scale_color_manual(values = f2colors32,
                    guide = guide_legend(reverse = TRUE)) +
  geom_text(aes(y = c(0.05,0.45), x = c(2,3),                 
                label = round(fig3t$result[c(1,2)],3)), 
            family = "Times New Roman",
            hjust = 0,
            size = 5) + 
  theme_classic() +
  ylab("Group Average of\nAbsolute Differences in Odds-Ratios") +
  xlab(" ") +
  #labs(title = "A. Raw Effect-Level Results, N = 3,742") +
  ...
  annotate("text", x = 1.5, 
           ...
           family = "Times New Roman",
           color = "gray30") +
  annotate("text", x = c(2,3), y = c(-0.05,-0.05), 
           label = c("TG", "OG"),
           family = "Times New Roman",
           hjust = 0.1, 
           size = 5) +
...
fig2bN_dev <- ggplot(data = fig2b[c(1,2),]) +
  coord_flip() +
  geom_errorbar(aes(ymin = deviance_fn1-(1.96*(deviance_fn2/sqrt(n_fn1))), ymax = deviance_fn1+(1.96*(deviance_fn2/sqrt(n_fn1))), x = c(2,3),), color = "grey30", width = 0.3) +
  geom_point(aes(y = deviance_fn1, x = c(2,3), color = group, size = 5)) +
  scale_x_reverse() +
  scale_color_manual(values = f2colors32,
                    guide = guide_legend(reverse = TRUE)) +
  geom_text(aes(y = c(0.05,0.9), x = c(2,3),                 
                label = round(fig3bt$result[c(1,2)],3)), 
            family = "Times New Roman",
            hjust = 0,
            size = 5) + 
  theme_classic() +
  ylab("Group Average of Team Averages\nin Absolute Differences in Odds-Ratios") +
  xlab(" ") +
  coord_flip(xlim = c(3.5,1), 
             ylim = c(-0.08,1),  
             clip = "on") +
