...
Based on peer review feedback we decided it is pertinent to generate a set of results where the three most deviant verification result teams were excluded from both groups (roughly 7% of each group). This carries the logic that teams that were unmotivated, unskilled or made major mistakes would probably not get their results published in a journal or in a study if they were subject to peer review, editor review or if seen by the original study's authors. It also reflects that in the opaque group there were three cases of questionable methods competencies.
...
team_trim <- unique(cri_team_trimmed$u_teamid[cri_team_trimmed$count < 4])
