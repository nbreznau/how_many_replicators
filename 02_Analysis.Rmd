...
pacman::p_load("dplyr", "readr", "lattice", "tidyr", "readxl", "knitr", "boot", "ragg", "kableExtra", "ggpubr","lme4", "jtools","sjPlot", "sjmisc", "sjlabelled", "rvest", "lavaan", "lavaanPlot", "see", "ggtext")
...
t_rct[,1] <- c("Stata","Stat Skill","Degree","Stata")
rm(cri2)
...
fig2[5:8,2] <- fig2[1:4,3] # get a long form
fig2[5:8, "stat"] <- "Exact Replication"

fig2[9:12,2] <- fig2[1:4,4] # get a long form
...
fig2$result <- fig2$verif_fn1*100
...
ggplot(data = fig2) +
  ...
  scale_fill_manual(values = f2colors,
                    labels = c("Curated Results, Opaque Group",
                               "Curated Results, Transparent Group",
                               "Original Results, Opaque Group",
                               "Original Results, Transparent Group"),
                    guide = guide_legend(reverse = TRUE)) +
  geom_vline(aes(xintercept = 4.75)) +
  #annotate("text", x = 2.5, y = 34, label = "Verification\n(same sign)", angle = 90) +
  geom_richtext(aes(x = 2.5, y = 30, label = "VERIFICATION<br>*(same sign)*"), 
                angle = 90, label.padding = unit(10, "pt")) +
  geom_richtext(aes(x = 7, y = 31, label = "EXACT<br>REPLICATION<br>*(within 0.01)*"), 
                angle = 90, label.padding = unit(10, "pt")) +
